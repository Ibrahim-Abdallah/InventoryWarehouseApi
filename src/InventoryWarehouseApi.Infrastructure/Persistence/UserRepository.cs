...
internal sealed class UserRepository(InventoryWarehouseDbContext db):IUserRepository
{
 ...
 public async Task RevokeActiveTokensAsync(Guid id,DateTimeOffset now,CancellationToken ct){var tokens=await db.RefreshTokens.Where(x=>x.UserId==id&&x.RevokedAtUtc==null).ToListAsync(ct);foreach(var t in tokens.Where(x=>x.IsActive(now)))t.Revoke(now);}
